...
f_xx + f_yy + x y f_xy + x f_x + y f_y + (x^2 + y^2) f
  - x/2 (f_x)^2 - y/2 (f_y)^2 + f_x f_y = Source(x,y)
...
source(x,y) = exp(-x^2 - y^2) * (-4 + 3 * (x^2 + y^2) + 4 * x^2 * y^2) -
    2 * exp(-2*x^2 - 2*y^2) * (x^3 + y^3 - 2 * x * y)


function compute_residual(Dxx::SparseMatrixCSC, Dyy::SparseMatrixCSC, Dxy::SparseMatrixCSC,
...
=#
function deriv_matrices(Dx_op, Dy_op, Dxx_op, Dyy_op)
...
Dx_op, Dy_op, Dxx_op, Dyy_op = deriv_operators(hx, hy, Nx, Ny, ord)
Dx, Dy, Dxx, Dyy, Dxy        = deriv_matrices(Dx_op, Dy_op, Dxx_op, Dyy_op)
