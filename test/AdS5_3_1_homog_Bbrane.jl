...
@time @testset "Homogeneous Black brane tests:" begin
    ...
    # gauge
    xi     = copy(gauge.xi[1,1,1]) #due to homogeneity; change for non-homog.
    Du_in  = systems[1].Du
    Du_out = systems[2].Du
    ...
    uu_in    = copy(chart_in.coords[1][:])

    A_in  = copy(bulk_in.A[:,1,1]);
    Au_in = copy(Du_in*A_in);
    ...
    for i in 2:num_out_domains+1
        ...
        uu_out_i    = copy(chart_out_i.coords[1][:])
        append!(uu_out, [uu_out_i])

        A_out_i  = copy(bulk_out_i.A[:,1,1])
        Au_out_i = copy(Du_out*A_out_i)
        append!(A_out,  [A_out_i])
        append!(Au_out, [Au_out_i])
    ...
    A_in_end  = copy( (1/(uu_in[end]^2) + 2.0*xi/uu_in[end] +   xi^2 + uu_in[end]^2 * A_in[end]) )
    Au_in_end = copy( (-2/(uu_in[end]^3) - 2.0*xi/(uu_in[end]^2)  + 2.0* uu_in[end]* A_in[end] + (uu_in[end]^2)* Au_in[end]) )
