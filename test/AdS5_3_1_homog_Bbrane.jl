...
@time @testset "Homogeneous Black brane tests:" begin
    ...
    # take u-derivatives
    Du_A_in    = Du_in * getA(bulk_in)
    Du_A_out_1 = Du_out * getA(bulk_out_1)


    # Compare value of A and its u-derivative on the inner-outer domain interface

    xi0         = getxi(gauge)[1,1,1]
    A0_in       = getA(bulk_in)[:,1,1]
    A0_out      = getA(bulk_out_1)[:,1,1]
    ...
    for i in 2:grid.u_outer_domains
        ...
        # take u-derivatives
        Du_A_1    = Du_1 * getA(bulk_1)
        Du_A_2    = Du_2 * getA(bulk_2)

        # 1-2 interface
        @test all(getA(bulk_1)[end,:,:] .≈ getA(bulk_2)[1,:,:])
    ...
    A_in    = getA(bulk_in)[:,i,j]
    A_u_in  = 2 .* uu_in .* getA(bulk_in)[:,i,j] .+ uu_in .* uu_in .* Du_A_in[:,i,j]
